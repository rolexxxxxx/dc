...
*/
int
connect_to_port (int connect_to)
{
  int sock_id;
  int opt = 1;
...
*/
void
send_to_id (int to, int id, char message[ML])
{
  ...
  sendto (id,
	  (const char *) message,
	  strlen (message),
	  MSG_CONFIRM, (const struct sockaddr *) &cl, sizeof (cl));
}
...
*/
int
election (int id, int *procs, int num_procs, int self)
{
  int itr;
  char message[ML];
...
*/
void
announce_completion (int id, int *procs, int num_procs, int self)
{
  int itr;
  char message[ML];
...
// 3. if not the initiator wait for someone else
  while (TRUE)
      ...
      if (!strcmp (buff, "ELECTION"))
	{
	  ...
	  sendto (sock_id, (const char *) message, strlen (message),
		  MSG_CONFIRM,
